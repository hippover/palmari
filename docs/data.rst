...
This class scans all ``.tif`` files in ``data_folder`` and stores localizations and tracks generated by :ref:`pipelines <image_pipeline>` in the ``export_folder``.
...
Once you have set up your `ImagePipeline <image_pipeline>`_, you might want to run it on all your movies. To do so, simply run the following command. 
...
It must be instanciated with a reference to a ``ImagePipeline`` responsible for generating or loading its localizations.
